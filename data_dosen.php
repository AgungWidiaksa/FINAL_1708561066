...
require 'php.php'; 
    ...
    if(isset($_POST['cari']) AND $_POST['filter']==""){
         echo '<script language="javascript">alert("Masukan Filter Search!"); document.location="data_dosen.php";</script>';
    }
    if (isset($_POST["submit"]) AND isset($_POST['filter']) AND isset($_POST["sort"])){
        $sort =  $_POST["sort"]; 
        $filter  = $_POST['filter'];
        $result = sorting1($filter, $sort);        
    }
    if(isset($_POST['submit']) AND isset($_POST['cari']) AND isset($_POST['filter']) AND isset($_POST["sort"])){
        $cari = $_POST['cari'];
        $filter  = $_POST['filter'];
        $sort =  $_POST["sort"]; 
        $result = carifiltersort1($cari,$filter, $sort);
    }
?>
    ...
    <body>
        ...
        <form action="" method="POST">
            ...
            <select class="select" name="filter"/>
                ...
                <option value="NIP">Filter By NIP</option>
